...
class FileObjectController extends Controller
{
    ...
    public function viewFolder($folderHash) {
        ...
        $objects = FileObject::whereLike('storage_path', "{$currentBasePath}/%")->get();
        $files = collect([]);
        $folders = collect([]);
    ...
    public function uploadFile(Request $request) {
        $request->validate([
            ...
            'file_object_sector_id' => 'nullable|exists:file_object_sectors,id',
        ...
        FileObject::create([
            'user_id' => Auth::id(),
            'file_object_sector_id' => $request->input('file_object_sector_id'),
