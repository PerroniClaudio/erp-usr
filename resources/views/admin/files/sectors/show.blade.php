        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.sectors.update', $sector) }}" method="POST">
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="text" name="name" class="input w-full" value="{{ old('name', $sector->name) }}"
                        required placeholder="{{ __('files.sectors_new_name_label') }}">
                </fieldset>
                ...
                <fieldset class="fieldset">
                    ...
                    <textarea name="description" class="textarea w-full" required
                        placeholder="{{ __('files.sectors_new_description_label') }}">{{ old('description', $sector->description) }}</textarea>
                </fieldset>

                <fieldset class="fieldset">
                    <legend class="fieldset-legend">{{ __('files.sectors_new_acronym_label') }}</legend>
                    <input type="text" name="acronym" class="input w-full"
                        value="{{ old('acronym', $sector->acronym) }}" required
                ...
                <fieldset class="fieldset">
                    ...
                    <div class="color-picker-wrapper flex items-center gap-3">
                        ...
                        <input type="color" name="color" class="sr-only" value="{{ old('color', $sector->color) }}"
                            required>
                    </div>
                </fieldset>
