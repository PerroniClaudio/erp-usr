<x-layouts.app>
    <div class="flex justify-between items-center">
        <h1 class="text-4xl">{{ __('overtime_requests.type') }}: {{ $overtimeRequest->overtimeType->name ?? '' }}
            ({{ $overtimeRequest->overtimeType->acronym ?? '' }})</h1>
    </div>
    <hr>
    <div class="card bg-base-300 w-full md:w-1/2 xl:w-1/3">
        <div class="card-body flex flex-col gap-2 ">
            <fieldset class="fieldset">
                ...
                <div class="p-2 bg-base-200 rounded">{{ $overtimeRequest->user->name }}</div>
            </fieldset>
            <fieldset class="fieldset">
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                @if ($overtimeRequest->status == 0 || $overtimeRequest->status == 1)
                    <form class="w-full" method="POST"
                        action="{{ route('admin.overtime-requests.approve', $overtimeRequest) }}"
                        style="display:inline-block;">
                        @csrf
                        <button type="submit"
                            class="btn btn-success w-full">{{ __('overtime_requests.approve') }}</button>
                    </form>
                    <form class="w-full" method="POST"
                        action="{{ route('admin.overtime-requests.deny', $overtimeRequest) }}"
                        style="display:inline-block;">
                        @csrf
                        <button type="submit"
                            class="btn btn-warning w-full">{{ __('overtime_requests.deny') }}</button>
                    </form>
                @endif
            </div>
